...
#define char POS_STAT[5]  //Position of person, will either be NORM (normal), LYFD (Lying Facedown), LYFU (Lying Faceup), LYRS (Lying Right Side), LYLS (Lying Left Side)
...
//--------------------- CHECK FOR CONDITIONS OF STRESS -------------------
  ...
  if(abs(mma.z) > 3100) {horizontal++; POS_STAT = "LYFD";}
  else if(abs(mma.z) < -3100) {horizontal++; POS_STAT = "LYFU";}
  else if(abs(mma.y) > 3100) {horizontal++; POS_STAT = "LYLS";}
  else if(abs(mma.y) < -3100) {horizontal++; POS_STAT = "LYRS";}
  else {horizontal = 0; POS_STAT = "NORM";}
